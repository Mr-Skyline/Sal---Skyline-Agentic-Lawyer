...
from src.sal.gmail_retry import gmail_execute
from src.sal.logger_util import log_event
from src.sal.secrets_store import (
...
class TestGmailExecute:
    ...
    @patch("src.sal.gmail_retry.time.sleep")
...
class TestLogEvent:
    def test_writes_jsonl(self):
        with tempfile.NamedTemporaryFile(
            ...
            log_path = f.name
        try:
            with patch("src.sal.logger_util.LOG_FILE", log_path):
    ...
    def test_no_crash_on_bad_path(self):
        with patch("src.sal.logger_util.LOG_FILE", "/nonexistent_dir/log.jsonl"):
