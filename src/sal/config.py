...
Repository root is derived from this file location (`src/sal/config.py`).
Optional: set **INTENDED_PROJECT_ROOT** in the environment to the folder you treat
as the canonical checkout (e.g. one machine path); `verify_setup` warns when the
running tree does not match.
"""
...
def _resolved_intended_root() -> Path:
    raw = os.environ.get("INTENDED_PROJECT_ROOT", "").strip()
    if raw:
        return Path(os.path.expandvars(os.path.expanduser(raw))).resolve()
    return ROOT.resolve()


INTENDED_PROJECT_ROOT = _resolved_intended_root()
...
# Sal Phase 2 markdown exports (`review_export.py`). Override with absolute path if needed.
