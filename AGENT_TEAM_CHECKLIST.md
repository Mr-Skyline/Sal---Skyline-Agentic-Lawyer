...
| 2026-04-09 | `AGENTS.md` refresh   | Aligned Cloud section with package layout: `scripts/smoke_check.py`, `python -m src.sal.verify_setup`, optional `scripts/env_check.py`; hello-world import `src.sal.analysis`; pytest note updated; Python 3.11+ + canonical root vs Cloud VM warning. |
...
| 2026-04-08 | CI triggers           | `.github/workflows/ci.yml`: `push` only on `main` (PRs still run via `pull_request`); avoids duplicate matrix runs when pushing feature branches. |
| 2026-04-08 | Ruff                  | `pyproject.toml`: `[tool.ruff.lint]` for select/ignore (replaces deprecated top-level keys); `ruff check --fix` on `main.py`, `src/sal/gmail_retry.py`, tests for I001 + dropped unused `io` in `test_evidence.py`. |
